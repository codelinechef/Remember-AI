...
{formatted}
    ...
    # to use them.
    msg = await llm.bind_tools([tools.upsert_memory, tools.search_memories]).ainvoke(
...
async def store_memory(state: State, runtime: Runtime[Context]):
    ...
    # Build coroutines for recognized tools
    coros = []
    valid_calls = []
    ...
    # Concurrently execute all recognized tool calls
    outputs = await asyncio.gather(*coros) if coros else []
    ...
    results = [
        ...
        for tc, out in zip(valid_calls, outputs)
    ]
